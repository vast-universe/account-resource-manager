    ...
    def get_user_info(self):
        ...
        Returns:
            tuple: (success, data_or_error)
                - success=True: data 包含 country, email 等信息
                - success=False: data_or_error 是错误消息
        """
        ...
        try:
            ...
            if response.status_code != 200:
                return False, f"获取用户信息失败: HTTP {response.status_code}"

            data = response.json()
            self._log(f"✓ 获取用户信息成功: country={data.get('country')}, email={data.get('email')}")
            return True, data

        except Exception as e:
            return False, f"获取用户信息异常: {e}"

    def create_checkout_session(self, country, currency, plan_name="chatgptplusplan", access_token=None, promo_campaign_id=None, workspace_name=None):
        """
        创建支付会话

        Args:
            country: 国家代码（如 'NL', 'US'）
            currency: 货币代码（如 'EUR', 'USD'）
            plan_name: 套餐名称，默认 'chatgptplusplan' (Plus)
            access_token: 访问令牌（可选，如果不提供则尝试从 session 获取）
            promo_campaign_id: 促销活动 ID（如 'plus-1-month-free'）
            workspace_name: Team计划的工作区名称（可选，默认为 'Team Workspace'）

        Returns:
            tuple: (success, data_or_error)
                - success=True: data 包含 checkout_session_id, processor_entity 等
                - success=False: data_or_error 是错误消息
        """
        url = f"{self.BASE}/backend-api/payments/checkout"

        # Team计划需要特殊处理
        is_team_plan = plan_name == "chatgptteamplan"

        payload = {
            "entry_point": "team_workspace_purchase_modal" if is_team_plan else "all_plans_pricing_modal",
            "plan_name": plan_name,
            "billing_details": {
                "country": country,
                "currency": currency
        ...
        # Team计划需要额外的team_plan_data字段
        if is_team_plan:
            payload["team_plan_data"] = {
                "workspace_name": workspace_name or "Team Workspace",
                "price_interval": "month",
                "seat_quantity": 5
            }
            payload["cancel_url"] = f"{self.BASE}/?promo_campaign={promo_campaign_id}#pricing" if promo_campaign_id else f"{self.BASE}/#pricing"

        # 添加促销活动参数
        if promo_campaign_id:
            payload["promo_campaign"] = {
                "promo_campaign_id": promo_campaign_id,
                "is_coupon_from_query_param": is_team_plan
            }
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return False, f"创建支付会话失败: HTTP {response.status_code}{error_detail}"
            ...
            if not checkout_session_id or not processor_entity:
                return False, f"响应缺少必要字段: {data}"

            # 构建支付链接
            checkout_url = f"{self.BASE}/checkout/{processor_entity}/{checkout_session_id}"
            data['checkout_url'] = checkout_url

            self._log(f"✓ 创建支付会话成功")
            self._log(f"  checkout_session_id: {checkout_session_id}")
            self._log(f"  processor_entity: {processor_entity}")
            self._log(f"  checkout_url: {checkout_url}")

            return True, data

        except Exception as e:
            return False, f"创建支付会话异常: {e}"
